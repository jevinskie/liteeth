...
class MDIOClient:
    ...
    def __init__(self, bus: RemoteClient):
        ...
        self.w = None
        self.r = None
        for attr in vars(bus.regs).keys():
            if attr.endswith("mdio_w"):
                self.w = getattr(bus.regs, attr)
            if attr.endswith("mdio_r"):
                self.r = getattr(bus.regs, attr)
            if self.w and self.r:
                break
        assert self.w and self.r
